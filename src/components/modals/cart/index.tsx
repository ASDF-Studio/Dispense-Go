...
import {
    deleteProductsByDispensaryId,
    emptyCart,
    setShowDelete,
    setShowVariants,
    toggleCartModal,
    updateVariantByProductId,
...
export const CartModal: FC<Props> = ({ open = false, setOpen }) => {
    ...
    const {
        ...
        showDelete,
        variantListUpdate: {
            productId: productIdForUpdatingVariant,
            variationList,
        },
    } = useAppSelector((state) => state.cart)

    const [variation, setVariation] = useState<string>("z")
            ...
            <FlexColumn className='h-full relative'>
                <FlexColumn className='px-6 pt-6 h-full'>
                    <FlexColumn className='gap-6 h-full'>
                        <Flex className='items-center justify-between'>
                            <Typography
                                intent={"grskt18"}
                                classname='leading-[18px] tracking-[-0.63px] font-medium text-text-black-100'
                            >
                                Basket ({items.length})
                            </Typography>
                            <IconButton
                ...
                <Link href='/checkout'>
                    <Button
                        ...
                        textClassname={[
                            "leading-[15px] font-medium uppercase",
                            isCartEmpty && "text-text-black-40",
                        ].join(" ")}
                        classname={[
                            "rounded-none w-full",
                            isCartEmpty && "bg-primary-brand/10",
                        ].join(" ")}
                    />
                </Link>
                ...
                {showVariants && (
                    <FlexColumn className='absolute top-0 h-full w-full bg-text-black-20'>
                        <FlexColumn className='absolute w-full h-[371px] bg-white bottom-0 rounded-t-[16px] p-6 gap-6'>
                            <FlexColumn className='gap-4'>
                                <Typography
                                    classname='uppercase leading-[18px] font-medium tracking-[-0.63px] text-text-black-100'
                                    intent={"grskt18"}
                                >
                                    select variant
                                </Typography>

                                {variationList.map((variant, id) => {
                                    return (
                                        <Pressable
                                            onClick={() =>
                            ...
                            <FlexColumn className='gap-4'>
                                <Button
                                    text='confirm'
                                    onClick={() => {
                                        dispatch(setShowVariants(false))
                                        dispatch(
                                            updateVariantByProductId(
                                                productIdForUpdatingVariant,
                                                variation
                                            )
                                        )
                                    }}
                    ...
                    <FlexColumn className='absolute top-0 h-full w-full bg-text-black-20'>
                        <FlexColumn className='absolute p-6 top-1/2 -translate-y-1/2 w-[357px] bg-white left-1/2 -translate-x-1/2 rounded-[4px] gap-6'>
                            <FlexColumn className=' gap-4'>
                                <Typography
                                    intent={"grskt18"}
                                    classname='font-medium tracking-[-0.63px] leading-[18px] text-text-black-100 uppercase text-center'
                                >
                                    remove product
                                </Typography>
                                <Typography
                                    intent={"mons12"}
                                    classname='font-medium leading-[15.6px] text-center text-text-black-70'
                                >
                                    You are about to remove this product from
                                    your basket. Are you sure to remove it?
                                </Typography>
                            </FlexColumn>
                            <FlexColumn className='gap-4'>
                                <Button
