...
const OrderState: FC<OrderStateType> = ({ state = "completed", classname }) => {
    const stateMapping = {
        completed: {
            text: <Typography intent={"mons15"} classname="leading-[19.5px] font-medium text-primary-green">Completed</Typography>,
            icon: <Image src={"/assets/CIRCLE-CHECK.png"} width={18} height={20} alt="icon" />,
        },
        ready: {
            text: <Typography intent={"mons15"} classname="leading-[19.5px] font-medium text-primary-green">Ready for Pick Up</Typography>,
            icon: <Image src={"/assets/BOX.png"} width={18} height={20} alt="icon" />,
        },
        failed: {
            text: <Typography intent={"mons15"} classname="leading-[19.5px] font-medium text-text-red">Payment Failed</Typography>,
            icon: <Image src={"/assets/CIRCLE-XMARK.png"} width={18} height={20} alt="icon" />,
        },
    };

    return <Flex className={["items-center gap-2", classname].join(" ")}>{stateMapping[state].icon} {stateMapping[state].text}</Flex>;
};
...
const OrderId: FC<OrderIdType> = ({ state = "success" }) => {
    return <Flex className={["py-2 px-3", state === "success" ? "bg-background-lightGreen" : "bg-text-red/10"].join(" ")}>
        <Typography
            classname={["leading-[14px] font-semibold tracking-normal", state === "success" ? "text-primary-green" : "text-text-red"].join(" ")}
            intent={"mons14"}
        >
...
const OrderHeader: FC<OrderHeaderType> = ({ state = "completed", onShowQR }) => {
    const stateMapping = {
    ...
    return (
        <Flex className="p-6 px-5 m:px-6 justify-between border-b border-b-background-lightGreen items-start xl:items-center flex-col m:flex-row gap-4">
            ...
            <Flex className="gap-4 items-center">
                ...
                {
                    state === "ready" &&
                    <Button text="QR code" onClick={onShowQR} intent={"filled"} typographyVariant="buttons" classname="shrink-0" />
                }
                {
                    state === "completed" &&
                    <Button text="write review" intent={"filled"} typographyVariant="buttons" classname="shrink-0" />
                }
                {
                    state === "failed" &&
                    <Button text="Pay now" intent={"outline"} typographyVariant="buttons" classname="shrink-0" />
                }
            </Flex>
        </Flex>
    );
...
const OrderBody = () => {
    return <FlexColumn className="p-6 px-5 m:px-6 gap-6 border-b border-b-text-black-10">
        <Typography
            intent={"mons15"}
...
const OrderFooter: FC<OrderHeaderType> = ({ state }) => {
    return <Flex className="py-6 px-5 xl:px-[42px] justify-between m:flex-row flex-col-reverse gap-y-4">
        <Flex className="gap-2 m:gap-4 flex-col m:flex-row">
            {state === "ready" && <Button intent={"outline"} text="cancel order" classname="w-full" typographyVariant="grstk15" textClassname="leading-[15px] tracking-[2.175px] uppercase text-black" />}
            <Button intent={"outline"} text="contact store" classname="w-full" typographyVariant="grstk15" textClassname="leading-[15px] tracking-[2.175px] uppercase text-black" />
        </Flex>

        <Flex className="items-center gap-2 justify-end">
            <Typography intent="mons15" classname="leading-[19.5px] font-medium text-text-black-100 tracking-normal">
                Order total
            </Typography>
